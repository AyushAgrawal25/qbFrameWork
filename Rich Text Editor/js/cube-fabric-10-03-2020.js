...
myApp.controller('qbCtrl', ['$scope', '$compile', 'qbAccordion', function($scope,$compile,qbAccordion) {
	$scope.qbSwitchValue=false;
}]);
...
qbEditorTools.$inject=['$compile','qbBasics'];
function qbEditorTools($compile,qbBasics)
{
    return {
		scope:{},
		transclude:true,
		link: function(scope, element, attr, ctrl, transclude){
			...
			var qbCmds=[
				{cmd:"bold", icon:"fas fa-bold"}
			];

			scope.qbCmdClickFun=function(qbCmd){
				console.log(qbTextArea[0].querySelector("iframe").contentWindow.document.body.innerHTML);
			}
			...
			angular.forEach(qbCmds, function(qbCmdVal, qbCmdKey){
				...
				angular.element(element).children().append($compile(qbCmdBtn)(scope));
			});
		},
		template: function(element,attr){
			return "<div class=\"qb-editor-tools\" ng-transclude>"+
...
myApp.directive('qbSwitch',['qbDataBase','qbBasics', function(qbDataBase,qbBasics) {
    return {
		...
		template: function(element,attr){	
            return  "<div style=\"display: inline-block; align-items: center; width: 20%; overflow: hidden;\" class=\"\" >"+
						"<div style=\"width: -webkit-fill-available;\" class=\"\">"+
							"<div class=\" \" style=\"background-color:"+attr.qbToggleSwitchColor+"; border-radius: 1000000000px; padding: 9%; width: auto; overflow: hidden;\">"+
								"<div class=\"\" style=\"position: relative; width: auto; overflow: hidden;\" >"+
									"<input ng-model=\"qbModel\" style=\"height: 21px; width: 21px; z-index: 1000; opacity: 0; position: absolute;\" type=\"checkbox\" class=\"qb-toggle-switch \">"+
									"<div style=\"height: 21px; width: 21px; border-radius: 50%; position: absolute;\" class=\"\" id=\"qb-toggle-circle\">"+
										
									"</div>"+
									"<div style=\"height: 21px; width: 21px; background-color: black; border-radius: 50%; opacity: 0;\" class=\" \">"+
...
myApp.directive('qbImgCropStage',['qbDataBase','qbBasics',function(qbDataBase,qbBasics){
	return {
		...
		link: function(scope,element,attr,ctrl,transclude){
			...
			var qbImgScaleDimensions=angular.element(qbBasics.findChildren(qbImgCropScale,"qb-img-scale-dimensions"));
			...
			var qbImgCropBoxExtremeSetFun=function(){
				var qbCropBoxMaxWidth=0;
				var qbCropBoxMaxHeight=0;
				var qbCropBoxMinWidth=0;
				var qbCropBoxMinHeight=0;
				
				if(scope.qbImgCropBoxProp.maxHeight)
				{
					if(scope.qbImgCropBoxProp.maxHeight.includes("%"))
					{
						qbCropBoxMaxHeight=parseFloat(scope.qbImgCropBoxProp.maxHeight)*parseFloat(window.getComputedStyle(qbImgCropStageCover[0], null).getPropertyValue('height'))/100;
					}
					if(scope.qbImgCropBoxProp.maxHeight.includes("px"))
					{
						qbCropBoxMaxHeight=parseFloat(scope.qbImgCropBoxProp.maxHeight);
					}
				}

				if(scope.qbImgCropBoxProp.maxWidth)
				{
					if(scope.qbImgCropBoxProp.maxWidth.includes("%"))
					{
						qbCropBoxMaxWidth=parseFloat(scope.qbImgCropBoxProp.maxWidth)*parseFloat(window.getComputedStyle(qbImgCropStageCover[0], null).getPropertyValue('width'))/100;
					}
					if(scope.qbImgCropBoxProp.maxWidth.includes("px"))
					{
						qbCropBoxMaxWidth=parseFloat(scope.qbImgCropBoxProp.maxWidth);
					}
				}

				if(scope.qbImgCropBoxProp.minHeight)
				{
					if(scope.qbImgCropBoxProp.minHeight.includes("%"))
					{
						qbCropBoxMinHeight=parseFloat(scope.qbImgCropBoxProp.minHeight)*parseFloat(window.getComputedStyle(qbImgCropStageCover[0], null).getPropertyValue('height'))/100;
					}
					if(scope.qbImgCropBoxProp.minHeight.includes("px"))
					{
						qbCropBoxMinHeight=parseFloat(scope.qbImgCropBoxProp.minHeight);
					}
				}

				if(scope.qbImgCropBoxProp.minWidth)
				{
					if(scope.qbImgCropBoxProp.minWidth.includes("%"))
					{
						qbCropBoxMinWidth=parseFloat(scope.qbImgCropBoxProp.minWidth)*parseFloat(window.getComputedStyle(qbImgCropStageCover[0], null).getPropertyValue('width'))/100;
					}
					if(scope.qbImgCropBoxProp.minWidth.includes("px"))
					{
						qbCropBoxMinWidth=parseFloat(scope.qbImgCropBoxProp.minWidth);
					}
				}

				qbImgCropBoxExtreme.maxHeight=qbCropBoxMaxHeight;
				qbImgCropBoxExtreme.minHeight=qbCropBoxMinHeight;
				qbImgCropBoxExtreme.maxWidth=qbCropBoxMaxWidth;
				qbImgCropBoxExtreme.minWidth=qbCropBoxMinWidth;
			...
			scope.qbImgCropBoxSizeSetFun=function(qbBoxWidth,qbBoxHeight){
				...
				//Checking And Setting new values
				if((qbImgCropBoxExtreme.minHeight<=qbBoxHeightInPx)&&(qbImgCropBoxExtreme.maxHeight>=qbBoxHeightInPx))
				{
					if((qbImgCropStageOffset.endPoint.y-qbImgCropBoxOffset.startPoint.y)<qbBoxHeightInPx)
					{
						qbBoxHeightInPx=qbImgCropStageOffset.endPoint.y-qbImgCropBoxOffset.startPoint.y;
					}
					qbImgCropBox.css("height",qbBoxHeightInPx+"px");
				}

				if((qbImgCropBoxExtreme.minWidth<=qbBoxWidthInPx)&&(qbImgCropBoxExtreme.maxWidth>=qbBoxWidthInPx))
				{
					if((qbImgCropStageOffset.endPoint.x-qbImgCropBoxOffset.startPoint.x)<qbBoxWidthInPx)
					{
						qbBoxWidthInPx=qbImgCropStageOffset.endPoint.x-qbImgCropBoxOffset.startPoint.x;
					}
					qbImgCropBox.css("width",qbBoxWidthInPx+"px");
				}
			...
			var qbImgCropStageLoadFun=function(){
				...
				//Getting extreme values on resize
				qbImgCropBoxExtremeSetFun();
